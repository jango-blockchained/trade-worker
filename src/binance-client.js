...
export class BinanceClient {
  ...
  async setLeverage(symbol, leverage) {
    try {
      ...
      const response = await fetch(
        `${this.baseUrl}/fapi/v1/leverage?${queryString}&timestamp=${timestamp}&signature=${signature}`,
        {
          method: "POST",
          headers: {
  ...
  async executeTrade({ symbol, side, orderType, quantity, price, reduceOnly }) {
    try {
      ...
      const response = await fetch(
        `${this.baseUrl}/fapi/v1/order?${queryString}&timestamp=${timestamp}&signature=${signature}`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  async getPositions(symbol = null) {
    try {
      ...
      const response = await fetch(
        `${this.baseUrl}/fapi/v2/positionRisk?${queryString}timestamp=${timestamp}&signature=${signature}`,
        {
          method: "GET",
          headers: {
            "X-MBX-APIKEY": this.apiKey,
