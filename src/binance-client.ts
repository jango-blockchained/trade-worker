...
export class BinanceClient implements IBinanceClient {
     ...
     */
    private async generateSignature(params: Record<string, string | number | boolean>): Promise<string> {
    ...
    private async makeRequest<T>(
        ...
        console.log("Binance Response Body:", JSON.stringify(responseData, null, 2));

        if (!response.ok) {
            const error = responseData as BinanceErrorResponse;
             throw new Error(`Binance API Error (${error.code}): ${error.msg || 'Unknown error'}`);
        }
