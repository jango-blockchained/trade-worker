...
export class BybitClient implements IBybitClient {
     ...
     */
    private async generateSignature(timestamp: number, paramsStr: string): Promise<string> {
    ...
    async executeTrade(params: { symbol: string; side: string; orderType: string; quantity: number; price?: number; reduceOnly?: boolean }): Promise<any> {
        ...
        const apiParams: Record<string, any> = {
            ...
            side: params.side.charAt(0).toUpperCase() + params.side.slice(1).toLowerCase(), // Bybit uses "Buy" or "Sell"
            orderType: params.orderType.charAt(0).toUpperCase() + params.orderType.slice(1).toLowerCase(), // "Market" or "Limit"
    ...
    async closeLong(symbol: string, quantity: number): Promise<any> {
        return this.executeTrade({ symbol, side: 'Sell', quantity, orderType: 'Market', reduceOnly: true });
    }

    async closeShort(symbol: string, quantity: number): Promise<any> {
        return this.executeTrade({ symbol, side: 'Buy', quantity, orderType: 'Market', reduceOnly: true });
    }
