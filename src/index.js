...
async function handleRequest(request, env) {
  ...
  try {
    ...
    // Initialize MEXC client with keys from environment variables
    const mexcClient = new MexcClient(env.MEXC_API_KEY, env.MEXC_API_SECRET);
    
    const { exchange, action, symbol, quantity, price, orderType = 'MARKET', leverage } = data;
    
    // Verify the exchange is supported
    ...
    if (leverage) {
      await mexcClient.setLeverage(symbol, leverage);
    }
    
    // Execute the trade
    const result = await mexcClient.executeTrade({
      symbol,
      side,
      orderType,
      quantity,
