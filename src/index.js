...
async function handleProcessRequest(request, env) {
  ...
  try {
    ...
    try {
      ...
      console.log(
        `Trade execution result for ${incomingRequestId}:`,
        tradeResult
      );
      ...
      console.error(
        `Trade execution failed for ${incomingRequestId}:`,
        tradeError
      );
    ...
    console.error(
      `Unexpected error processing request ${incomingRequestId}:`,
      error
    );
