...
import worker, { validateApiCredentials, validateTradePayload, saveReportToR2 } from './index'; // Import the worker and helpers
...
const mockBinanceClient = { ...mockMexcClient }; // Create separate mock objects
const mockBybitClient = { ...mockMexcClient };
...
describe('Trade Worker Helpers', () => {
  ...
  let validateTradePayload: (payload: any) => { isValid: boolean; error?: string };
  let saveReportToR2: (reportData: any, payload: any, dbLogId: number | null, env: any) => Promise<void>;
  ...
  describe('validateApiCredentials', () => {
    ...
    it('should return false if key binding itself is missing for bybit', async () => {
        ...
        expect(await validateApiCredentials('bybit', envWithoutBinding)).toBe(false);
    });
  ...
  describe('validateTradePayload', () => {
     ...
     it('should return invalid if payload is null or not object', () => {
         expect(validateTradePayload(null)).toEqual({ isValid: false, error: 'Invalid or missing payload' });
         expect(validateTradePayload("string")).toEqual({ isValid: false, error: 'Invalid or missing payload' });
     });
     it('should return invalid for missing required fields', () => {
         expect(validateTradePayload({ ...validPayload, symbol: undefined })).toEqual({ isValid: false, error: 'Missing required fields in payload' });
     });
     it('should return invalid for invalid action', () => {
         expect(validateTradePayload({ ...validPayload, action: 'HOLD' })).toEqual({ isValid: false, error: 'Invalid action in payload: HOLD' });
     });
     it('should return invalid for non-positive quantity', () => {
         expect(validateTradePayload({ ...validPayload, quantity: 0 })).toEqual({ isValid: false, error: 'Invalid quantity in payload' });
         expect(validateTradePayload({ ...validPayload, quantity: -1 })).toEqual({ isValid: false, error: 'Invalid quantity in payload' });
         expect(validateTradePayload({ ...validPayload, quantity: 'abc' })).toEqual({ isValid: false, error: 'Invalid quantity in payload' });
     });
     it('should return invalid for invalid price', () => {
         expect(validateTradePayload({ ...validPayload, price: 'abc' })).toEqual({ isValid: false, error: 'Invalid price in payload' });
     });
     it('should return invalid for invalid leverage', () => {
          expect(validateTradePayload({ ...validPayload, leverage: 'abc' })).toEqual({ isValid: false, error: 'Invalid leverage in payload' });
          expect(validateTradePayload({ ...validPayload, leverage: 0 })).toEqual({ isValid: false, error: 'Invalid leverage in payload' });
          expect(validateTradePayload({ ...validPayload, leverage: 10.5 })).toEqual({ isValid: false, error: 'Invalid leverage in payload' }); // Must be integer
     });
     it('should return valid if optional fields (price, leverage, orderType) are missing', () => {
      ...
      it('should return valid if optional fields (price, leverage) are present and valid', () => {
         expect(validateTradePayload({ ...validPayload, price: 50000, leverage: 20 })).toEqual({ isValid: true });
     });
  });
    ...
    it('should call R2 put with correct key and data', async () => {
     ...
     it('should log error and not call put if R2 binding is missing', async () => {
      ...
      expect(errorSpy).toHaveBeenCalledWith(expect.stringContaining(`[${dbLogId}] REPORTS_BUCKET binding is not configured.`));
...
describe('Trade Worker Handlers', () => {
    ...
    beforeEach(() => {
        ...
        mockMexcClient.executeTrade.mockClear();
        mockBinanceClient.executeTrade.mockClear();
        mockBybitClient.executeTrade.mockClear();
        mockMexcClient.setLeverage.mockClear();
        mockLogRequest.mockClear();
        mockLogResponse.mockClear();

        // Default successful mocks
        mockLogRequest.mockResolvedValue(logId);
        mockMexcClient.executeTrade.mockResolvedValue({ orderId: 'mexc123' });
        mockBinanceClient.executeTrade.mockResolvedValue({ orderId: 'bin987' });
        mockBybitClient.executeTrade.mockResolvedValue({ orderId: 'byb456' });
    ...
    describe('/webhook handler', () => {
        it('should validate payload, log, init client, set leverage, execute trade, log response, and save report', async () => {
            ...
            expect(mockLogResponse).toHaveBeenCalledWith(logId, expect.any(Response), null, startTime);

            // Check client init
            expect(mockMexcClientConstructor).toHaveBeenCalledWith('mexc-key', 'mexc-secret');
            ...
            expect(mockMexcClient.setLeverage).toHaveBeenCalledWith(validPayload.symbol, validPayload.leverage);
            expect(mockMexcClient.executeTrade).toHaveBeenCalledWith(expect.objectContaining({
                symbol: validPayload.symbol,
                side: 'BUY', // Mapped from LONG
                quantity: validPayload.quantity,
                orderType: 'MARKET', // Default if not provided
            }));

            // Check R2 save
            expect(mockEnv.REPORTS_BUCKET.put).toHaveBeenCalledWith(
                 expect.stringContaining(`trade-reports/mexc/BTC_USDT/${logId}.json`),
                 JSON.stringify({ orderId: 'mexc123' }, null, 2) // The result from executeTrade
            );
        });
        ...
        it('should return 400 if payload validation fails', async () => {
            ...
            expect(mockMexcClient.executeTrade).not.toHaveBeenCalled();
        });
        ...
        it('should return 400 if API credentials are not configured for the exchange', async () => {
            ...
            const body = await response.json();
            expect(body.success).toBe(false);
            expect(body.error).toContain('not configured for exchange: mexc');
        });
         ...
         it('should handle errors during setLeverage gracefully', async () => {
            ...
            expect(response.status).toBe(200); // Still proceeds to trade
             expect(mockMexcClient.executeTrade).toHaveBeenCalled(); // Trade should still be attempted
             expect(errorSpy).toHaveBeenCalledWith(expect.stringContaining('Failed to set leverage'), leverageError);
             errorSpy.mockRestore();
        });

         it('should return 500 if executeTrade fails', async () => {
            const tradeError = new Error('Trade execution failed');
            mockMexcClient.executeTrade.mockRejectedValue(tradeError);
            const request = createMockRequest('POST', '/webhook', validPayload);
            ...
            const body = await response.json();
            expect(body.success).toBe(false);
            expect(body.error).toBe(tradeError.message);
            expect(mockLogResponse).toHaveBeenCalledWith(logId, expect.any(Response), tradeError, expect.any(Number));
         ...
         it('should select and use Binance client based on payload', async () => {
            ...
            expect(mockBinanceClientConstructor).toHaveBeenCalledWith('binance-key', 'binance-secret');
            expect(mockBinanceClient.executeTrade).toHaveBeenCalled();
            expect(mockMexcClientConstructor).not.toHaveBeenCalled();
            expect(mockBybitClientConstructor).not.toHaveBeenCalled();
        });
         ...
         it('should skip leverage setting if leverage not in payload', async () => {
            ...
            expect(mockMexcClient.executeTrade).toHaveBeenCalled();
        });
    ...
    describe('/process handler', () => {
         ...
         it('should return 401 if internalAuthKey is missing or incorrect', async () => {
         ...
         it('should return 400 if nested payload is invalid', async () => {
            const invalidNestedPayload = { ...processPayload, payload: { ...validPayload, action: 'INVALID'} };
             const request = createMockRequest('POST', '/process', invalidNestedPayload);
             const response = await worker.fetch(request, mockEnv, { waitUntil: vi.fn() } as any);
             expect(response.status).toBe(400);
             const body = await response.json();
