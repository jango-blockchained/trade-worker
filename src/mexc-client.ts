...
export interface IMexcClient {
    setLeverage(symbol: string, leverage: number, positionSide?: string): Promise<any>;
    executeTrade(params: { symbol: string; side: string; orderType: string; quantity: number; price?: number; reduceOnly?: boolean }): Promise<any>;
    getAccountInfo(): Promise<any>;
    getPositions(symbol?: string): Promise<any>;
    openLong(symbol: string, quantity: number, price?: number, orderType?: string): Promise<any>; // Helper
    openShort(symbol: string, quantity: number, price?: number, orderType?: string): Promise<any>; // Helper
...
export class MexcClient implements IMexcClient {
     ...
     */
    private async makeRequest<T>(method: string, path: string, params: Record<string, string | number> = {}): Promise<T> {
        ...
        if (method === 'GET' || method === 'DELETE') {
             ...
             Object.entries(allParams).forEach(([key, value]) => url.searchParams.append(key, String(value)));
        ...
        if (!response.ok || responseData.code !== 200) {
            throw new Error(`MEXC API Error (${responseData.code}): ${responseData.msg || 'Unknown error'}`);
        }
     ...
     */
    async setLeverage(symbol: string, leverage: number, positionSide: string = 'BOTH'): Promise<any> {
     ...
     */
    async executeTrade(params: { symbol: string; side: string; orderType: string; quantity: number; price?: number; reduceOnly?: boolean }): Promise<any> {
        const path = "/api/v1/private/order/submit"; 
        const apiParams: Record<string, string | number> = {
            symbol: params.symbol,
            side: params.side.toUpperCase() === 'BUY' || params.side.toUpperCase() === 'LONG' ? 1 : 2, // 1: Open long, 2: Open short, 3: Close long, 4: Close short
        ...
        if (apiParams.type === 1 && params.price) { // Only add price for LIMIT orders
    ...
    async openLong(symbol: string, quantity: number, price?: number, orderType: string = "MARKET"): Promise<any> {
        return this.executeTrade({ symbol, side: 'BUY', quantity, price, orderType });
    }

    async openShort(symbol: string, quantity: number, price?: number, orderType: string = "MARKET"): Promise<any> {
        return this.executeTrade({ symbol, side: 'SELL', quantity, price, orderType });
    }

    async closeLong(symbol: string, quantity: number): Promise<any> {
        // Assuming closing uses a market order
        return this.executeTrade({ symbol, side: 'CLOSE_LONG', quantity, orderType: 'MARKET', reduceOnly: true });
    }

    async closeShort(symbol: string, quantity: number): Promise<any> {
        // Assuming closing uses a market order
        return this.executeTrade({ symbol, side: 'CLOSE_SHORT', quantity, orderType: 'MARKET', reduceOnly: true });
    }
