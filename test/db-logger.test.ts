...
describe("DbLogger", () => {
    ...
    beforeEach(() => {
        ...
        mockD1ServiceFetch.mockResolvedValue(
            new Response(JSON.stringify({ success: true, lastRowId: LOG_REQUEST_ID }), {
                status: 200,
    ...
    test("should disable logging and warn if D1_SERVICE binding is missing", () => {
        ...
        expect(warnSpy).toHaveBeenCalledWith("D1_SERVICE binding not found. Database logging disabled.");
        warnSpy.mockRestore();
    });
    ...
    test("logRequest should call D1_SERVICE fetch with correct SQL and params", async () => {
        ...
        const requestHeaders = { "content-type": "application/json", "cf-connecting-ip": "1.2.3.4", "user-agent": "TestAgent" };
    ...
    test("logRequest should return null if D1_SERVICE fetch fails (non-ok status)", async () => {
        mockD1ServiceFetch.mockResolvedValueOnce(new Response("D1 Error", { status: 500 }));
        ...
        expect(result).toBeNull();
        expect(errorSpy).toHaveBeenCalledWith("Failed to log request via D1_SERVICE:", "D1 Error");
    ...
    test("logRequest should return null if D1_SERVICE fetch throws an error", async () => {
        ...
        expect(result).toBeNull();
        expect(errorSpy).toHaveBeenCalledWith("Error logging request via D1_SERVICE:", fetchError);
        errorSpy.mockRestore();
    });
    ...
    test("logResponse should call D1_SERVICE fetch with correct SQL and params", async () => {
        ...
        const response = new Response(responseBody, { status: responseStatus, headers: responseHeaders });
        const startTime = Date.now() - 150; // 150ms execution time
        const error = null;

        // Reset fetch mock for the response log call (assume success)
        mockD1ServiceFetch.mockResolvedValueOnce(new Response(JSON.stringify({ success: true }), { status: 200 }));
    ...
    test("logResponse should handle D1_SERVICE fetch errors gracefully", async () => {
        ...
        // Should not throw
        await expect(logger.logResponse(LOG_REQUEST_ID, response)).resolves.toBeUndefined();

        expect(errorSpy).toHaveBeenCalledWith("Failed to log response via D1_SERVICE:", "D1 Error");
        errorSpy.mockRestore();
    });
     ...
     test("logResponse should handle fetch throwing an error gracefully", async () => {
        ...
        expect(errorSpy).toHaveBeenCalledWith("Error logging response via D1_SERVICE:", fetchError);
        errorSpy.mockRestore();
    });
