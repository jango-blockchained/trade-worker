...
describe("Trade Worker", () => {
    ...
    const createMockEnv = (secrets, mocks = {}) => ({
        ...
        MEXC_SECRET_BINDING: { get: jest.fn().mockResolvedValue(secrets.mexcSecret) },
        BINANCE_KEY_BINDING: { get: jest.fn().mockResolvedValue(secrets.binanceKey ?? null) },
        BINANCE_SECRET_BINDING: { get: jest.fn().mockResolvedValue(secrets.binanceSecret ?? null) },
        BYBIT_KEY_BINDING: { get: jest.fn().mockResolvedValue(secrets.bybitKey ?? null) },
        BYBIT_SECRET_BINDING: { get: jest.fn().mockResolvedValue(secrets.bybitSecret ?? null) },
    ...
    test("rejects request with invalid internal key config", async () => {
        ...
        mockEnv = createMockEnv({ mexcKey: TEST_MEXC_KEY, mexcSecret: TEST_MEXC_SECRET, internalKey: null });
    ...
    test("rejects request if API key bindings not configured", async () => {
        ...
        mockEnv = createMockEnv({ internalKey: TEST_INTERNAL_KEY, mexcKey: null, mexcSecret: null });
        const request = new Request("https://trade-worker.workers.dev", {
            method: "POST",
        ...
        expect(body.error).toContain("API secret bindings not configured or accessible for mexc");
        expect(MockMexcClient).not.toHaveBeenCalled(); // Check constructor mock
        expect(mockExecuteTrade).not.toHaveBeenCalled();
    });
    ...
    test("executes long position on MEXC successfully", async () => {
        ...
        expect(MockMexcClient).toHaveBeenCalledWith(TEST_MEXC_KEY, TEST_MEXC_SECRET);

        // Verify client methods were called (using the stand-alone mocks)
        expect(mockSetLeverage).toHaveBeenCalledWith("BTC_USDT", 20);
        expect(mockExecuteTrade).toHaveBeenCalledWith(expect.objectContaining({ symbol: "BTC_USDT", side: "BUY", quantity: 0.1 }));
